...
const Signup = () => {
  ...
  const verifyOtpMutation = useMutation({
    ...
    onSuccess: (data) => {
      console.log("Verify seller response:", data);
      setSellerId(data?.seller?.id);
      console.log("Setting sellerId to:", data?.seller?.id);
      setActiveStep(2);
    },
    onError: (error: AxiosError) => {
      const errorMessage = (error.response?.data as { message: string })
  ...
  return (
    <div className="w-full flex flex-col items-center pt-10 min-h-screen">
          ...
          <div key={step}>
            <div
              className={`w-10 h-10 rounded-full text-white font-bold flex items-center justify-center ${
                step <= activeStep ? "bg-black " : "bg-gray-300"
              }`}
            >
              {step}
            </div>
      ...
      <div className="md:w-[480px] p-8 bg-white rounded-lg shadow">
        ...
        {activeStep === 2 && (
          <>
            {console.log("Rendering CreateShop with sellerId:", sellerId)}
            <CreateShop
              sellerId={sellerId || ""}
